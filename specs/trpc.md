...
### 1. `app/trpc/init.ts` — Inicializacao do tRPC
...
### 2. `app/trpc/routers/_app.ts` — Root router
...
### 3. `app/trpc/routers/roast.ts` — Procedures de roast
...
### 4. `app/trpc/routers/leaderboard.ts` — Procedures de leaderboard
...
### 5. `app/trpc/query-client.ts` — QueryClient factory
...
### 6. `app/trpc/client.tsx` — Client provider
...
### 7. `app/trpc/server.tsx` — Server-side helpers
...
### 8. `app/api/trpc/[trpc]/route.ts` — HTTP handler
...
### 9. `app/layout.tsx` — Adicionar provider
...
- [x] Criar `app/trpc/routers/_app.ts` — appRouter mergeando sub-routers
- [x] Criar `app/trpc/query-client.ts` — makeQueryClient factory
- [x] Criar `app/trpc/client.tsx` — TRPCReactProvider, useTRPC
- [x] Criar `app/trpc/server.tsx` — createTRPCOptionsProxy, HydrateClient, prefetch, caller
- [x] Criar `app/api/trpc/[trpc]/route.ts` — fetch adapter handler
- [x] Modificar `app/layout.tsx` — adicionar TRPCReactProvider envolvendo children
- [x] Substituir dados hardcoded de stats em `app/page.tsx` por prefetch de roast.getStats
- [ ] Substituir dados hardcoded em `app/page.tsx` por prefetch de leaderboard.list
- [ ] Substituir dados hardcoded em `app/leaderboard/page.tsx` por prefetch de leaderboard.list
- [ ] Substituir dados hardcoded em `app/roast/[id]/page.tsx` por prefetch de roast.getById
