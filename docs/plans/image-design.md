...
- `app/roast/[id]/opengraph-image/route.ts` — GET route handler returning `new ImageResponse(...)` as PNG
- `components/og/roast-og-image.tsx` — JSX component describing the OG image layout (uses Takumi `tw` props)

### Modified files

- `app/roast/[id]/page.tsx` — Replace static `metadata` export with `generateMetadata()` function
